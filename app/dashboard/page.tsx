...
export default function SubAdminDashboard() {
  // 🌟 ไม่ต้องแก้รหัส UUID แล้วครับ! ล็อกแค่ชื่องานแต่ง (Slug) ตรงนี้พอ ระบบจะไปหา ID มาให้เองอัตโนมัติ
  const [slug] = useState("wedding-test");
  ...
  const [agendaItems, setAgendaItems] = useState([
    ...
    { id: 5, py: "20:15", task: "พิธีตัดเค้กแต่งงาน / โยนดอกไม้เสี่ยงทายดวงชะตา", done: false },
    { id: 6, time: "21:00", task: "After Party ปลดล็อกเครื่องยนต์เพลงแดนซ์สะเทือนเบส", done: false }
  ]);

  // 📡 เครื่องยนต์ดึงรหัส UUID ประจำหน้าจออัตโนมัติข้ามอากาศเมื่อเปิดหน้าเว็บ
  useEffect(() => {
  ...
  // 🚀 ท่ออัปโหลดเวอร์ชันแก้ไข evening เป็น finally สมบูรณ์ร้อยเปอร์เซ็นต์
  const handleUploadMediaFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      if (!screenId) {
        setUploadStatus("❌ ยังดึงเลขรหัสหน้าจอไม่สำเร็จ กรุณารอสักครู่หรือรีเฟรชหน้าต่าง");
        return;
      }
      ...
      setUploadStatus("กำลังบันทึกพิกัดลงดาต้าเบส...");
      ...
      setUploadStatus("🎉 สำเร็จ! ภาพจะวิ่งสับเปลี่ยนขึ้นเวทีโดยอัตโนมัติ");
    } catch (err: any) {
      console.error(err);
      setUploadStatus("❌ พังชั่วคราว: " + (err.message || "เกิดข้อผิดพลาดด้านเครือข่าย"));
    } finally {
      // 🟢 ปรับปรุง: เปลี่ยนจากคำว่า evening กลับมาเป็นคำสั่งสากล finally เรียบร้อยครับ!
      setUploading(false);
    }
  };
          ...
          <div className="bg-slate-900/60 border border-white/5 p-5 rounded-2xl space-y-4 shadow-xl">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <span className="text-cyan-400 font-mono font-black">คลังที่เลือก: {targetPoolKey}</span>
              </div>
